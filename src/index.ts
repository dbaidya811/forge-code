...
function renderTurnStats(agent: AgentExecutionLoop, mode: string): void {
...
export async function startApp(): Promise<void> {
  ...
  console.log(
    `${theme.dim('Commands:')} ${theme.terracotta('/model')} ${theme.dim('· switch model')} ${theme.dim('│')} ${theme.terracotta('/mode')} ${theme.dim('· switch mode')} ${theme.dim('│')} ${theme.terracotta('/clear')} ${theme.dim('│')} ${theme.terracotta('exit')}\n`
  );
  ...
  while (true) {
    ...
    const input = (rawInput as string).trim();
    ...
    if (command === '/clear' || command === 'clear') {
      ...
      renderBigBanner();
      console.log(
        `${theme.dim('Commands:')} ${theme.terracotta('/model')} ${theme.dim('│')} ${theme.terracotta('/mode')} ${theme.dim('│')} ${theme.terracotta('/clear')} ${theme.dim('│')} ${theme.terracotta('exit')}\n`
      );
      isFirstTurn = true;
      continue;
    }
