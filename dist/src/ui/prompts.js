...
export async function promptRoutingMode(service) {
...
export async function promptSearchModel(service) {
    const s = p.spinner();
    ...
    try {
        const selectedModel = await search({
            ...
            source: async (input) => {
                ...
                const filtered = allModels.filter((m) => m.id.toLowerCase().includes(query) || m.name.toLowerCase().includes(query));
